...
/*   Updated: 2023/02/09 21:23:38 by pfrances         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_redirections(t_cmd *cmd)
{
	...
	i = 0;
	while (g_state.error_state == NO_ERROR && cmd->redirect[i] != NULL)
	{
	...
	if (g_state.error_state == NO_ERROR && cmd->input_fd != STDIN_FILENO)
	{
		cmd->input_fd_save = dup(STDIN_FILENO);
		dup2(cmd->input_fd, STDIN_FILENO);
		close(cmd->input_fd);
	}
	if (g_state.error_state == NO_ERROR && cmd->output_fd != STDOUT_FILENO)
	{
